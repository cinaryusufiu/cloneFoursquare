...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    fileprivate func rootViewController() {
        ...
        window?.makeKeyAndVisible()
         GMSServices.provideAPIKey(AppConstant.ApiKeyValue.googleApiKey)
        Thread.sleep(forTimeInterval: 2)
        
        self.window?.rootViewController = UINavigationController(rootViewController: SearchViewController())
            //UINavigationController(rootViewController: PlaceViewController(placeViewModel: PlaceViewModel(venues: [Venue]())))
      /*
